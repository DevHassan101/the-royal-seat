<x-app-layout>
    <div class="flex justify-between items-center w-full mb-6">
        <div class="ml-1">
            ...
            <p class="text-gray-500 text-sm">View complete driver information and ITC permit data</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span class="text-sm font-medium text-gray-900">{{ $driver->email }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Emirates ID</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->emirates_id ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    @if ($driver->driverInfo?->itc_status)
                        @switch($driver->driverInfo->itc_status)
                            @case('pending')
                                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800 uppercase">{{ $driver->driverInfo->itc_status }}</span>
                                @break
                            @case('approved')
                                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 uppercase">{{ $driver->driverInfo->itc_status }}</span>
                                @break
                            @default
                                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 uppercase">{{ $driver->driverInfo->itc_status }}</span>
                        @endswitch
                    @else
                ...
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_permit_number ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Permit Type</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_permit_type ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Permit Issue Date</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_permit_issue_date?->format('d M Y') ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Permit Expiry Date</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_permit_expiry_date?->format('d M Y') ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">TCF Number</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_tcf_number ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Driver Name (ITC)</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_driver_name ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Nationality</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_nationality ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">License Issue Place</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_license_issue_place ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">License Expiry</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_license_expiry_date?->format('d M Y') ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Emirates ID Expiry</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_emirates_id_expiry_date?->format('d M Y') ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Date of Birth</span>
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_date_of_birth?->format('d M Y') ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Eligible for Trip</span>
                    @if ($driver->driverInfo?->itc_is_eligible_for_trip === true)
                        <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Yes</span>
                    @elseif ($driver->driverInfo?->itc_is_eligible_for_trip === false)
                ...
                <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <span class="text-sm font-medium text-gray-900">{{ $driver->driverInfo?->itc_last_synced_at?->format('d M Y H:i:s') ?? 'Never' }}</span>
                </div>
                <div class="flex justify-between items-start py-3">
                    <span class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Remarks</span>
                    <span class="text-sm font-medium text-gray-900 text-right max-w-xs">{{ $driver->driverInfo?->itc_remarks ?? 'N/A' }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mt-6 flex items-center gap-4">
        <form action="{{ route('itc.sync-driver', $driver->driverInfo->id) }}" method="POST">
            @csrf
            <button type="submit"
