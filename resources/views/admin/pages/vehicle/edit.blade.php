<x-app-layout>
    <div
        ...
        <div class="ml-1">
            ...
            <p class="text-gray-500 text-sm">Update the vehicle information</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
        ...
        <!-- Form Body -->
        <form action="{{ route('vehicle.update', ['vehicle' => $vehicle]) }}" method="post" class="p-6"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @selected(old('category', $vehicle->vehicle_category_id) == $category->id)>{{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('category')
                ...
                <div>
                    ...
                    <select
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" @selected($user->id == old('driver', $vehicle->user_id))>{{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('driver')
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" step="any" name="per_day_charges" id="per_day_charges"
                            ...
                            value="{{ old('per_day_charges', $vehicle->per_day_charges) }}">
                    </div>
                ...
                <div>
                    ...
                    <select
                        ...
                        <option value="automatic" @selected(old('transmission', $vehicle->transmission))>Automatic</option>
                        <option value="manual" @selected(old('transmission', $vehicle->transmission))>Manual</option>
                    </select>
                    @error('transmission')
                        <p class="mt-1 text-sm text-red-500 flex items-center gap-1">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                ...
                <div class="col-span-1 sm:col-span-2">
                    ...
                    <div class="relative">
                        <textarea name="permit_details" id="permit_details" rows="5"
                            class="block w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#c9982b] focus:border-[#c9982b] transition-all duration-200 resize-none"
                            placeholder="Enter permit details, restrictions, or special notes...">{{ old('permit_details', $vehicle->permit_details) }}</textarea>
                    </div>
