                            ...
                            @foreach($expenses as $expense)
                                <tr class="border-b last:border-0 hover:bg-gray-50">
                                    ...
                                    <td class="py-3 text-right">
                                        @if($expense->receipt)
                                            <a href="{{ asset($expense->receipt) }}" target="_blank" class="inline-block p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-all" title="View Receipt">
                                        ...
                                        @endif
                                        <form action="{{ route('expense.destroy', $expense) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?')">
                                            @csrf
                                            @method('DELETE')
