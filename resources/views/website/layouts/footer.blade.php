<footer class="bg-gray-950 border-t border-yellow-500/40 text-gray-400">
    ...
    <div class=" px-24 py-16 grid grid-cols-1 md:grid-cols-4 gap-10">
        ...
        <div>
            ...
            <ul class="space-y-3">
                @foreach ([['Home', url('/')], ['About Us', url('/about-us')], ['Vehicles', url('vehicles')], ['Contact', url('contact-us')]] as $link)
                    <li>
                        <a href="{{ $link[1] }}"
            ...
            <ul class="space-y-3">
                @foreach (['Daily Car Rental', 'Luxury Cars', 'Corporate Rentals', '24/7 Support'] as $service)
                    <li class="group flex items-center gap-2 text-sm text-gray-500">
                        <span
                            class="w-1.5 h-1.5 rounded-full bg-yellow-500/40 group-hover:bg-yellow-400 transition-all duration-200 shrink-0"></span>
                        {{ $service }}
                    </li>
                @endforeach
            </ul>
        ...
        <div>
            ...
            <ul class="space-y-4 text-sm">
                ...
                <li class="flex items-center gap-3 text-gray-500 hover:text-gray-300 transition-colors duration-200">
                    <svg class="w-4 h-4 text-yellow-500 shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class="border-t border-gray-800">
        <div class="max-w-7xl mx-auto px-6 py-5 flex flex-col md:flex-row items-center justify-between gap-3">
            <p class="text-xs text-gray-600">
                © 2026 <span class="text-white font-semibold">THE ROYAL</span><span
                    class="text-yellow-500 font-semibold"> SEAT</span>.
                All rights reserved.
            </p>
            <p class="text-xs text-gray-700">Designed by <a
                    href="https://bytecloude-site-i9v0n5pwx-dev-hassans-projects-480c65d5.vercel.app/" target="_blank"
                    class="hover:text-[#facc15]">ByteCloude</a></p>
        </div>
    </div>
